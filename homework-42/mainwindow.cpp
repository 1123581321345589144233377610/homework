...
void MainWindow::StartRace(void){
    ...
    if(ui->rb_qtConcur->isChecked()){
        auto firstRace = [this](){ concurRace1->DoWork(&number, ui->rb_mutexOn->isChecked(), ui->sb_initNum->value());};
        auto secondRace = [this](){ concurRace2->DoWork(&number, ui->rb_mutexOn->isChecked(), ui->sb_initNum->value());};
        QFuture<void> future = QtConcurrent::run(firstRace).then(secondRace);
    }
    else{
